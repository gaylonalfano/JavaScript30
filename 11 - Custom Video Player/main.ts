...
(function () {
  ...
  const playToggleButton = player.querySelector(".toggle") as HTMLButtonElement;
  ...
  const rangeSliders = player.querySelectorAll(
    ".player__slider"
  ...
  function updatePlayButton() {
    ...
    const buttonText: string = this.paused ? "►" : "❚ ❚";
  ...
  function skip(e: Event) {
    ...
    const skipAmount = parseFloat(this.dataset.skip); // or Number()
  ...
  function updateProgressBarFilled(e: Event) {
    ...
    const currentProgressPercentage: number =
      Math.round((currentTime / duration) * 100 * 10) / 10; // WORKS! rounds to tenths
    const currentProgressPercentageString: string =
      currentProgressPercentage.toString() + "%";
  ...
  function adjustProgress(e: MouseEvent) {
    ...
    const maxX = progress.offsetWidth; // progress.offsetWidth also undefined? YES! But no TypeError! Weird...
    ...
    const newProgressFraction: number =
      Math.round((clickX / maxX) * 1000) / 1000; // thousandths
    const duration: number = video.duration; // number: 595.6
    const newCurrentTime: number = newProgressFraction * duration;
  ...
  skipButtons.forEach((btn: HTMLButtonElement) => {
  ...
  rangeSliders.forEach((slider: HTMLInputElement) => {
  ...
  progress.addEventListener(
    "mousemove",
    (e: MouseEvent) => isMousedown && adjustProgress(e)
  );
